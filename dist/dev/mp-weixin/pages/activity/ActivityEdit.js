...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props) {
    ...
    const editActivity = async (data) => {
      console.log("显示加载中");
      const res = await services_ActivityLaunch.editActivityAPI(data);
      console.log(res);
    ...
    const getOriginValue = async () => {
      const res = await services_ActivityLaunch.getActivityInfoAPI(props.id);
      content.value = {
        id: res.data.id,
      ...
      selectedColleges.value = res.data.deptActivities.map((item) => item.deptName);
      res.data.deptActivities.forEach((item) => {
        populationLimit.value.find((i) => i.college === item.deptName).population = item.maxNum;
      });
    };
    ...
    common_vendor.onUnload(() => {
      common_vendor.index.$emit("backPageData", { state: 1 });
    });
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        O: common_vendor.o(($event) => selectedColleges.value = $event),
        P: common_vendor.p({
          multiple: true,
        ...
        S: common_vendor.o(($event) => content.value.onlineAddress = $event),
        T: common_vendor.p({
        ...
        V: common_vendor.o(($event) => content.value.sort = $event),
        W: common_vendor.p({
          localdata: typeList,
