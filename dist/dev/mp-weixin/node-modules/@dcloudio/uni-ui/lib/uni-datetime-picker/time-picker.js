...
const { t } = common_vendor.initVueI18n(common_vendor.i18nMessages);
const _sfc_main = {
  ...
  watch: {
    modelValue: {
      handler(newVal) {
        if (newVal) {
          this.parseValue(common_vendor.fixIosDateFormat(newVal));
          this.initTime(false);
        } else {
          this.time = "";
    ...
    start: {
      handler(newVal) {
        this.parseDatetimeRange(common_vendor.fixIosDateFormat(newVal), "start");
      },
      immediate: true
    },
    end: {
      handler(newVal) {
        this.parseDatetimeRange(common_vendor.fixIosDateFormat(newVal), "end");
      },
      immediate: true
    },
  ...
  methods: {
    ...
    initTimePicker() {
      if (this.disabled)
        return;
      const value = common_vendor.fixIosDateFormat(this.time);
